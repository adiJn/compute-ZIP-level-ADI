...
SUPPRESSION_CODES <- c('PH', 'GQ', 'GQ-PH', 'KVM', 'QDI')

# invalid state 
INVALID_STATE_FIPS <- c("80", "81", "60", "90")
...
# source functions
source(file.path(cdir, "zip_blk_grp_overlap/functions/all_unzip.R"))
source(file.path(cdir, "zip_blk_grp_overlap/functions/read.R"))
source(file.path(cdir, "zip_blk_grp_overlap/functions/zip_level_pop_weighted_ADI.R"))
source(file.path(cdir, "zip_blk_grp_overlap/functions/blk_grp_pop_merge.R"))
...
# read ADI data
read() %>%
  # clean ADI data, pull population data from 5-year ACS 2020, and merge on FIPS
  blk_grp_pop_merge() %>%
  ...
  # crosswalk
  gen_categories_export_data(paste0(ddir, "/national_adi/crosswalk"))
toc()
